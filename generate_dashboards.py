...
"""
    # Replace the fetch(...) calls in refresh()
    refresh_js = f"""
...
"""
    ...
    html = re.sub(r'  async function refresh\(\) \{.*?(?=  function renderStatus)s\}', refresh_js + '\n', html, flags=re.DOTALL)
    
    # Replace other fetches
    html = re.sub(r'fetch\(\'/api/', f"fetchAPI('/api/", html)
    html = html.replace('const res = await fetchAPI', 'const res = await fetchAPI') # if any
    
    # Replace fetch(...) with fetchAPI(...) inside updateSequence, testPort, simulateKnock, revoke
    html = re.sub(r'await fetch\(\'/api/', r"await fetchAPI('/api/", html)
    # Remove .json() unwrapping because fetchAPI does it
  ...
  function renderStrikes(strikes) {
    ...
    tbody.innerHTML = strikes.map(s => {
      ...
      for (let i = 0; i < m; i++) {
        ...
        pips += `<div style="width: 20px; height: 20px; border-radius: 50%; border: 2px solid ${pipBorder}; background: ${pipColor};"></div>`;
      }
      ...
      if (logRows.length > 0 && logs && logs.length > 0) {
          ...
          logRows.forEach((row, i) => {
              if (reversed[i] && reversed[i].location) {
                  ...
                  let flag = loc.country_code === 'US' ? '🇺🇸 ' : (loc.country_code === 'RU' ? '🇷🇺 ' : '');
                  locSpan.textContent = flag + loc.country_name;
                  if (loc.is_blocked) {
...
print("Generated all dashboards.")
